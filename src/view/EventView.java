...
public class EventView extends JFrame implements ActionListener {
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource() == createEventButton) {
      if (!newEvent) {
        this.displayError("Cannot create an event that already exists");
        return;
      }
    ...
    if (e.getSource() == modifyEventButton) {
      if (newEvent) {
        this.displayError("Cannot modify an event that does not exist");
        return;
      }
    ...
    if (e.getSource() == removeEventButton) {
      if (newEvent) {
        this.displayError("Cannot remove an event that does not exist");
        return;
      }
  ...
  private void printEventDetails(StringBuilder details) {
    ...
    details.append("Online: ").append(onlineComboBox.getSelectedItem())
            .append(System.lineSeparator());
    if (newEvent) {
      details.append("Host: ").append(user).append(System.lineSeparator());
    } else {
      details.append("Host: ").append(event.getHost()).append(System.lineSeparator());
    }
